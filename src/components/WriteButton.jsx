...
export default class WriteButton extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      limit: this.props.limit || 750,
      type: this.props.type || "words",
      compact: true,
      ...
      <div className="session-chooser">
        <div className="compact" onClick={this.onExpand}>
          ...
          <span className="choice">
            {limit} {type} <i className="edit icon-pencil"></i>
          </span>
        </div>
      </div>
    );
  ...
  renderOptions() {
    ...
    if (
      ...
      !options.includes(this.props.limit)
    ) {
    ...
    return options.map((limit) => {
      const classes = classNames("radio", {
        active: limit === this.state.limit,
      });
      return (
        <span
          key={limit}
          className={classes}
          onClick={() => this.setLimit(limit)}
        >
          {limit}
        </span>
      );
    ...
    return (
      <div className="session-chooser">
        <div className={classes}>
          <div className="tabs">
            <span className="minutes" onClick={() => this.setType("minutes")}>
              Minutes
            </span>
            &nbsp;/&nbsp;
            <span className="words" onClick={() => this.setType("words")}>
              Words
            </span>
          </div>
          <div className="radios">{this.renderOptions()}</div>
          <div
            onClick={this.toggleHardcore}
            className={classNames("hardcore", { checked: this.state.hardcore })}
          >
            Hardcore mode
          </div>
        </div>
  ...
  render() {
    const wrapperWlasses = classNames("writeButton", {
